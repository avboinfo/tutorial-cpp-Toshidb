...
public:
    ...
    bool puoi_continuare()
    {
        ...
        if (hai_vinto)
        {
            std::cout << "Hai indovinato il codice segreto! Complimenti!" << std::endl;
            return true;
        }
        else if (numero_mosse >= NUMERO_MOSSE_CONSENTITE)
        {
            std::cout << "Hai esaurito il numero massimo di mosse consentite. Il codice segreto era: ";
            for (int i = 0; i < DIM_GIOCATA_VALIDA; i++)
            {
                std::cout << codice_segreto[i];
            }
